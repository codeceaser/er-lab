...
from ingestion_bench.hybrid_retrieval_benchmark.seed_provider import collect_hybrid_seeds
...
def _hybrid_metrics(seeds, graph_side: GraphSideResult, fused: list[FusedChunk], vector_ranked: list[RankedChunk], graph_ranked: list[RankedChunk]) -> dict[str, Any]:
    ...
    return dict(
        ...
        seed_source_contribution=contribution, query_time_embedding_calls=graph_side.embedding_calls,
    )
...
def run_probe(
    ...
    conditions = {
        "real_graph": (real_proj, InMemoryGraphStore()),
        "perfect_graph": (perfect_proj, InMemoryGraphStore()),
    }
    edge_indexes: dict[str, InMemoryEdgeSemanticIndex] = {}
    edge_index_manifests: dict[str, Any] = {}
    for name, (proj, store) in conditions.items():
        ...
        edge_indexes[name] = InMemoryEdgeSemanticIndex(records)
    ...
    for question in contract["questions"]:
        ...
        def _record(mode: str, cond: str, fused: list[FusedChunk], latency: float, hybrid: dict | None) -> None:
        ...
        # so the H2/Vector latency-gate ratio is honest.
        v_latency = vector_by_question[qid]["total_latency_seconds"]
        _record("V", "common", v_fused, v_latency, None)
        ...
        for cond, (proj, store) in conditions.items():
            ...
            g_latency = time.perf_counter() - g_start
            g_fused, g_ranked = _graph_fused(g_result, chunk_evidence)
            _record("G", cond, g_fused, g_latency, None)
            ...
            _record("H0", cond, h0, resolver_latency + g_latency,
                    _hybrid_metrics([], GraphSideResult([], [], [], {}, 0, 0, 0.0), h0, v_ranked, g_ranked))

            # expanded seeds for H1/H2
            seed_start = time.perf_counter()
            seeds = collect_hybrid_seeds(
                use_explicit=True, use_vector=True, use_semantic=True, query=question["query"], query_vector=query_vector,
                nodes=list(proj.nodes.values()), node_by_id=proj.nodes, chunk_evidence=chunk_evidence,
                eligible_revision_ids=bundle.eligible_union, eligible_edges=eligible_edges, edge_index=edge_index, top_k=top_k,
                vector_candidate_multiplier=params["vector_candidate_multiplier"], max_vector_seed_chunks=params["max_vector_seed_chunks"],
                semantic_edge_candidate_count=params["semantic_edge_candidate_count"],
            )
            seed_latency = time.perf_counter() - seed_start
            eligible_edge_hits = len(edge_index.semantic_search_eligible(query_vector=query_vector, eligible_revision_ids=bundle.eligible_union, top_n=params["semantic_edge_candidate_count"]))
            ...
            m1 = _hybrid_metrics(seeds, h1_side, h1, v_ranked, h1_ranked)
            m1["eligible_semantic_edge_hits"] = eligible_edge_hits
            _record("H1", cond, h1, resolver_latency + seed_latency + h1_side.latency_seconds, m1)

            # H2 -- RRF(V, semantic-path-ranked expanded-seed graph)
            h2_side = semantic_path_ranked_graph_evidence(seeds=seeds, eligible_edges=eligible_edges, node_by_id=proj.nodes, chunk_evidence=chunk_evidence, query_vector=query_vector, embedding_provider=embedding_provider, max_hops=params["max_hop_depth"], max_candidate_paths=params["max_candidate_paths"])
            ...
            m2 = _hybrid_metrics(seeds, h2_side, h2, v_ranked, h2_ranked)
            m2["eligible_semantic_edge_hits"] = eligible_edge_hits
            _record("H2", cond, h2, resolver_latency + seed_latency + h2_side.latency_seconds, m2)
    ...
    return ProbeRunResult(
        ...
        edge_index_manifests=edge_index_manifests, mode_results=mode_results, decision_gate=gate_id, decision=decision,
        decision_rationale=rationale, real_gate_inputs=json.loads(real_gate.model_dump_json()),
