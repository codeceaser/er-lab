"""Stage 7B.0: the ONE real, persisted cross-document vector store --
Postgres + pgvector, using its OWN isolated table
(`ingestion_bench_stage7b0_vectors` by default). Never Stage 7A.1's or
Stage 7R.2's tables, never the ER GraphRAG POC's tables.

The authority-aware primitive expresses the cross-document eligibility
union as a SQL `document_revision_id IN (...)` clause INSIDE the SAME
query that does `ORDER BY embedding <=> ... LIMIT ...` -- never a
separate unfiltered query filtered afterward in Python. Because retrieval
spans the whole corpus (all logical documents), the only scope on the
ranking query is `embedding_model` plus that eligibility IN-list.
"""
...
from ingestion_bench.cross_document_benchmark.config import DATABASE_URL, VECTOR_TABLE_NAME
from ingestion_bench.cross_document_benchmark.store import RevisionVectorRecord, SearchHit, UpsertResult, compute_index_hash


class PgVectorStoreUnavailable(RuntimeError):
    """Raised when DATABASE_URL is not configured or the database is not
    reachable. Callers catch this to skip gracefully -- never to silently
...
class PgVectorCrossDocumentStore:
    ...
    def _create_schema_if_needed(self) -> None:
        ...
        with self._engine.connect() as conn:
            ...
            conn.execute(
                text(
                    f"CREATE INDEX IF NOT EXISTS {self._table_name}_document_revision_idx "
    ...
    def search_eligible(
        ...
        if not eligible_revision_ids:
            return []
        engine = self._ensure_ready()
        # The cross-document eligibility restriction (document_revision_id
        # IN :ids) lives in the SAME query as ORDER BY .../LIMIT -- never a
        # separate, broader query filtered afterward in Python.
        stmt = text(
            f"""
