...
| 7C.2 | Wiki Hub Retrieval / Navigation Qualification: the frozen truth-free arms (D0 / W1-D / W1-FULL, plus V and W0), the three attribution deltas, and the counterfactual claim-suppression resilience probe | **Completed (measured, read-only) — stopped at the owner page-quality checkpoint** | `reports/stage7c_wiki_scorecard.md`, `reports/stage7c_wiki_results.json`, `reports/stage7c2_findings.html`, `reports/stage7c_page_quality_blind_packet.md`, `reports/stage7c_page_quality_rubric.json`, `scripts/run_stage7c2_wiki_retrieval.py`, `src/ingestion_bench/wiki_projection/{retrieval,navigation,stage7c2_report}.py` | `tests/test_wiki_retrieval.py` (+38 Stage 7C.2 tests; 1115 repo-wide, 12 skipped) — invariants only (frozen hashes, no reachable compiler, authority-before-LIMIT, scorer identity, zero leakage, truth unreachable), never a winner | Measured over 12 frozen questions with the frozen 7B.0 scorer by import identity; zero compiler/extractor calls, the 22 frozen facet vectors loaded and never regenerated, nothing in 7C.0/7C.1 written. **Outcomes:** V 9 solved / 3 partial; W0 9/3 (materially equivalent to V, as predeclared); D0 7 solved / 5 partial (qualifying); W1-D and W1-FULL each 6 solved / 5 partial / 1 failed, both `NON-QUALIFYING / DIAGNOSTIC ONLY`. **Authority leakage 0 across every arm and question.** **Attribution:** W1-D worse than D0 on 1 question; W1-FULL ≡ W1-D on every axis and question (claim-derived routing changed nothing); W1-FULL worse than D0 on 1 question; D0 worse than V on 4. Wiki arms reached 4–6 of the 6 eligible chunks — the loss is **selection under the zero-slack budget**, not reachability. Suppression probe (truth-informed, diagnostic only, not Gate-A admissible): suppressing 4/8/8 path-relevant claim links changed no outcome or coverage. **Open:** owner page-quality rating of the blind six-page packet, and the Gate B vs Gate C decision — `docs/STAGE7C_WIKI_DECISION.md` is **not** written |
| 7C | Wiki page/link projection | **In progress — 7C.0, 7C.1 and 7C.2 complete and frozen; Gate Q = FAIL, so Gate A is UNREACHABLE; Gate B vs Gate C not decided, pending owner page-quality rating** | — | — | Experimental contract frozen as `docs/STAGE7C_WIKI_PLAN.md` **Revision 6** (owner-approved). R6's core correction: R5 compared `N_W0` against `N_W1` while holding the W1 enriched facet seed constant in BOTH arms, so it could isolate claim-derived routing but could never show the compiler was unnecessary — the compiler was seeding both sides. R6 adds the **D0** arm (chunk-embedding seed, no W1-derived model output) and names three arms **D0 / W1-D / W1-FULL**; Gate A-7 now requires W1-FULL to beat **D0**, and Gate B may never be selected from `W1-FULL ≈ W1-D` alone. **7C.2 measured that no Wiki arm beat V and that W1 added no measured value over the deterministic D0 arm** (see the 7C.2 row); the decision doc is not written, so no Gate B/C outcome may be quoted. Terminology: never write "zero-model" for D0 — it uses the existing embedding model; use **zero-W1-LLM**, **no W1-derived model output**, or **deterministic D0** |
...
- Wiki page/link retrieval (Stage 7C — **in progress**, consuming Stage 7R's resolver at query time: 7C.0 deterministic projection **frozen**; 7C.1 bounded W1 compiler **closed and frozen** after owner adjudication of all 68 items, with **final Gate Q = FAIL** on Q-5/Q-7/Q-8, 22 final facet embeddings built; 7C.2 retrieval/navigation **measured, read-only, complete** — V 9 solved / 3 partial, W0 9/3, D0 7 solved / 5 partial, W1-D and W1-FULL each 6/5/1 and labelled `NON-QUALIFYING / DIAGNOSTIC ONLY`, zero authority leakage everywhere; **Gate A unreachable and the Gate B vs Gate C decision not made** — `docs/STAGE7C_WIKI_DECISION.md` does not exist, so no Stage 7C retain/retire outcome may be quoted)
...
Stage 7C       Wiki page/link projection                           <- IN PROGRESS
               ...
               7C.2 retrieval/navigation qualification             <- DONE (Gate A unreachable; B/C open)
