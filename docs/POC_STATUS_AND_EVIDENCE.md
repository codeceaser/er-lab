...
**Key documents:**
- `docs/CONSOLIDATED_FINDINGS_AND_LEARNINGS.md` — cross-cutting findings,
  known limitations, and transferable engineering learnings (Stages 7R.1 → 7B.2).
- `docs/DEVIN_REBUILD_PROMPT.md` — a self-contained (no-repo-access) spec
  to rebuild this system with every refinement/pushback already baked in.
- Per-stage decision docs: `docs/STAGE7B0_CROSS_DOCUMENT_QUALIFICATION.md`,
...
| 7R.1 | Revision Authority Registry and Effective-Knowledge Resolution: narrow registry reusing Stage 4.1's own chunk-lineage identity fields, mutable authority metadata kept separately, deterministic query-time resolver (current/as_of/comparison/draft), atomic supersession, append-only audit log, real Postgres + in-memory repositories | **Completed, superseded by 7R.1a/7R.1b, frozen with 7R.1b** | `src/ingestion_bench/revision_authority/*.py`, `scripts/run_stage7r1_revision_scenarios.py` | `tests/test_revision_authority_*.py` | Corrected by 7R.1a/7R.1b -- see below |
| 7R.1a | Effective-period, historical-withdrawal, and atomicity closure: `AuthorityPeriod` (own table, `edib_revision_authority_period`) is now the SOLE source for effective dates -- `AuthorityMetadata` no longer carries them; withdrawal now takes an explicit `withdrawal_effective_date` (never `recorded_at`) and is period-aware (historical dates before withdrawal still resolve effective); `reinstate_revision()` supports a second, disjoint period after a post-effective rollback; `activate_revision`/`reinstate_revision` fully validate (existence, same document, no self-supersession, exactly one open period, no overlap) before any write, with real snapshot/rollback (in-memory) and a real transaction (Postgres) proven by fault-injection tests; integrity validation now applies to all four query intents (comparison/draft exclude a malformed record individually, never hard-fail); duplicate requested revision IDs rejected deterministically; structured `decision_effective_date`/`closure_reason` on audit events | **Completed, superseded by 7R.1b, frozen with 7R.1b** | Same files as Stage 7R.1, patched; `contracts/revision_authority_scenarios_v1.json` (renamed to `_v2.json` in Stage 7R.1b) | `tests/test_revision_authority_*.py` (86, new/updated; includes two skippable real-Postgres tests, one proving a failed activation rolls back completely); `reports/stage7r1_revision_authority_{results.json,scorecard.md}` (regenerated from the same contract, never a second, independently-drifting copy) | Corrected by 7R.1b -- see below |
| 7R.1b | Narrower closure on top of 7R.1a: `record_authority_decision()` now only ever accepts `draft`/`under_review` at runtime (approved/withdrawn only via `activate_revision`/`reinstate_revision`/`withdraw_revision`); `activate_revision`/`reinstate_revision`/`withdraw_revision` no longer expose a public generic `closure_reason` (each can only ever produce its own specific reason); `register_revision`/`record_authority_decision` writes now also wrapped in `repository.transaction()`; ONE central `integrity.validate_document_integrity()` (new `integrity.py`) used by all four query intents, classifying every problem as revision-scoped (comparison/draft exclude individually) or document-scoped (hard-fails ALL FOUR intents, including comparison/draft -- the key behavioral change from 7R.1a); a genuine mid-transaction real-Postgres fault (not just a pre-write validation rejection) now proves real `ROLLBACK`; existing/legacy (pre-7R.1a-shape) Postgres schemas handled explicitly (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS` for the event table, `Stage7RLegacySchemaError` fail-fast for populated legacy registry columns with no matching period, `scripts/reset_stage7r_tables.py` one-time reset for disposable POC data); scenario contract renamed to `contracts/revision_authority_scenarios_v2.json`, scenario runner now requires EXACT equality (eligible revisions, excluded revision/reason pairs, full authority-label state set, integrity error code) -- an unexpected extra exclusion or label now fails a scenario. Two additional local invariant guards (never a separate stage): `record_authority_decision()` also rejects draft/under_review once a revision has ANY non-zero-width authority period (current, historical, or scheduled future -- zero-width corrections remain eligible); `withdraw_revision(closure_reason="correction")` now requires `withdrawal_effective_date == open_period.effective_from` exactly (a correction after even one real day must use `closure_reason="withdrawn"` instead) | **Completed, frozen** | Same files as Stage 7R.1/7R.1a, patched; `src/ingestion_bench/revision_authority/integrity.py` (new); `scripts/reset_stage7r_tables.py` (new); `contracts/revision_authority_scenarios_v2.json` (renamed, `contract_version: revision_authority_scenarios_v2`) | `tests/test_revision_authority_*.py` (110 total; +24 vs. 7R.1a: 14 from the original 7R.1b closure, +10 from the two local invariant guards); `reports/stage7r1_revision_authority_{results.json,scorecard.md}` regenerated from the renamed contract | None known -- see `docs/REVISION_AUTHORITY_SCENARIOS.md`. **Stage 7R.1/7R.1a/7R.1b is now frozen** |
| 7R.2 | Authority-Aware Vector Retrieval: proves Stage 7R.1's registry/resolver actually changes semantic-search results. Isolated benchmark over ONE logical document (POLICY-RETENTION-001, 5 near-identical real DOCX revisions through the frozen Stage 5A adapter + Stage 4/4.1 chunker), an isolated Postgres/pgvector table (never Stage 7A.1's own), a narrow authority-aware retriever (resolve_query_scope() -> fail closed on integrity_error -> SQL-level `WHERE document_revision_id IN (...)` eligibility filter BEFORE `ORDER BY`/`LIMIT` -- never post-filtered), current/historical/draft/comparison scenarios plus a live authority-switch scenario proving zero re-ingestion/re-chunking/re-embedding across an activation, and an honest unfiltered-vs-authority-aware comparison. **7R.2a correction**: removed a source-level authority leak (v4's DOCX carried its own "PROPOSED" sentence -- draft/effective/superseded labels now come ONLY from the resolver); every query artifact now persists the COMPLETE `AuthorityAwareSearchResult` (registry snapshot hash, eligible ids, excluded+reasons, all authority labels, integrity_error(+code), full ranked authority-aware AND unfiltered hits with complete provenance -- source_relative_path, unit_indices, heading_path, source_element_ids, full `source_refs`), never just derived metrics; added `embedding_payload_sha256` (chunk_id + content_sha256 + retrieval_text hash + actual stored vector, scoped by logical_document_id+embedding_model) proving the STORED EMBEDDING itself, not just chunk identity, survives an authority switch unchanged; the switch scenario now validates the contract's declared before/after authority labels against the resolver's actual labels; metrics renamed for accuracy (`ineligible_revision_leakage_at_k` -> `ineligible_hit_count_at_k`, `eligible_revision_precision_at_k` -> `eligible_hit_precision_at_k`, plus new `distinct_ineligible_revision_count_at_k`); every store operation now scoped explicitly by `logical_document_id` (SQL `WHERE logical_document_id = :logical_document_id`); generation_manifest.json now records each fixture's expected SHA-256 and fixtures.py fails loudly if a tracked DOCX's on-disk bytes ever diverge from it | **Completed, frozen** | `src/ingestion_bench/revision_search_benchmark/*.py`, `fixtures/revision_search/*`, `contracts/revision_search_benchmark_v1.json`, `scripts/run_stage7r2_authority_aware_benchmark.py` | `tests/test_revision_search_*.py` (29 total, was 21; includes one skippable real sentence-transformers+pgvector integration test); `reports/stage7r2_authority_aware_vector_{results.json,scorecard.md}` (real run: real Docling conversion, real sentence-transformers, real Postgres); `artifacts/stage7r2/{index_manifest,registry_before,registry_after}.json` + `query_results/*.json` (gitignored, like every other stage's `artifacts/` output -- regenerable from the same committed report data, never the source of truth itself) | None known -- unfiltered retrieval genuinely leaks ineligible revisions (ineligible_hit_count_at_k 6-8 of 10) even with the real embedding model and the corrected, leak-free fixtures, since the five documents are deliberately near-identical; authority-aware eligible_hit_precision_at_k is 1.00 in every scenario. **Stage 7R.2 is now frozen** |
| 7B.0 | Projection-Neutral Cross-Document Relationship Holdout and Vector Baseline: qualifies a benchmark that can later determine, fairly, whether an evidence-backed graph projection adds retrieval value beyond Vector RAG -- **without building any graph**. A deterministic enterprise corpus of 11 real DOCX files / 6 logical documents distributes the current chain APP-224510 -> Payment Settlement -> O-31 -> C-88 -> P-205 one hop per document (no chunk holds a pre-assembled multi-hop answer -- enforced by test); projection-neutral distractors: retired app APP-224499, superseded control C-88a, historical procedure P-204 (authority-historical), draft control C-91 (registered never activated), and a lexically similar authority-CURRENT "Payment Reconciliation" adjacent chain. A projection-neutral fact contract + 12-question contract (direct/one-hop/two-hop/multi-hop/consolidation/distractor/current-authority/historical/draft) hold all evaluation truth (required/forbidden facts, expected chain) that ingestion/retrieval never read. A NEW isolated `cross_document_benchmark` package: reuses (read-only) the frozen Stage 5A adapter, Stage 4/4.1 chunker, Stage 7R.1 resolver, Stage 7A.1 embedding provider, and Stage 7R.2 `RevisionVectorRecord` provenance schema; its cross-document retriever resolves EACH document via the frozen resolver, unions eligible revisions, and applies a SQL `document_revision_id IN (...)` filter (over the whole corpus) BEFORE `ORDER BY`/`LIMIT`. Per question: coverage@K, all-required@K, MRR, nDCG, authority-leakage, unfiltered-ineligible count, evidence-document diversity, complete-chain-represented, solved/partial/failed. Freezes a fairness contract for the future graph comparison (same docs/chunks/resolver/questions/budgets/provenance; no precomputed path; no eval-truth access during projection; every future node/edge must cite existing chunk_ids) | **Completed, frozen** | `src/ingestion_bench/cross_document_benchmark/*.py` (new package), `fixtures/cross_document/*` (new, 11 tracked DOCX + generator + manifest), `contracts/cross_document_relationship_benchmark_v1.json` (new), `scripts/run_stage7b0_cross_document_benchmark.py` (new), `docs/STAGE7B0_CROSS_DOCUMENT_QUALIFICATION.md` (new) | `tests/test_cross_document_{store,benchmark_integration}.py` (20, new; includes one skippable real sentence-transformers+pgvector integration test); `reports/stage7b0_cross_document_vector_{results.json,scorecard.md}` (real run: real Docling, real sentence-transformers, real Postgres); `artifacts/stage7b0/{fixture_manifest,evidence_alignment,vector_index_manifest}.json` + `query_results/*.json` (gitignored, regenerable) | Real run: authority correct 12/12 (zero leakage); Vector solved 9, partial 3 (Q04 two-hop, Q06 four-hop, Q07 consolidation -- each missed exactly the intermediate hop the query never names, out-ranked by the lexically-similar adjacent chunk), failed 0 -- the honest multi-hop gap a future graph would target. Known limitation: small corpus (only ~6 authority-eligible chunks per current query) makes the recall ceiling easy to reach, so absolute scores overstate large-corpus Vector performance; value is the methodology + fair frozen harness. No graph nodes/edges/traversal/answer-generation/wiki built. **Stage 7B.0 is now frozen** |
| 7B.1 | Evidence-Backed Graph Projection and Vector-vs-Graph Retrieval Comparison: builds a graph projection from the EXACT frozen Stage 7B.0 chunks (verified: source SHA-256, document_revision_ids, chunk_ids, and content hashes via the committed index_hash all re-checked before any graph work) and compares Graph retrieval against the frozen Stage 7B.0 Vector baseline under 7B.0's frozen fairness contract. NEW isolated `graph_retrieval_benchmark` package: a narrow relationship extractor (deterministic `FakeRelationshipExtractor` rule-parser for tests + `OpenAIRelationshipExtractor`, env-overridable model, strict JSON-schema output, full request/response/token/cost provenance) that sees ONLY chunk text/lineage (never the fact contract/questions); a revision-scoped `GraphEdgeAssertion` model (NO is_current/latest/effective/superseded, NO question ids/paths -- current/historical/draft never merged into one timeless edge) where every edge cites an existing chunk and every `supporting_text` is validated as an exact substring (unsupported edges rejected, never repaired); isolated Postgres tables (edib_stage7b1_graph_node/edge_assertion/extraction_run, NO Neo4j) + in-memory stores for tests; an authority-aware retriever that resolves each doc via the frozen 7R resolver, unions eligible revisions, loads ONLY eligible edge assertions (authority filter BEFORE traversal), seeds by exact/normalized alias match (never the expected chain), BFS to a fixed global hop cap (5) both directions, and ranks the frozen top-K UNIQUE supporting chunks by (hop distance, supporting-chunk similarity, id). Graph is scored by the SAME frozen 7B.0 `_evaluate_question` + `build_evidence_alignment` (import identity, proven by test), and compared against the frozen 7B.0 Vector results (loaded, never rerun/rescored). Graph construction never reads eval truth; a separate evaluator measures build accuracy (edge recall/precision, missing facts, unsupported edges, provenance completeness, invalid-chunk edges, entity-normalization collisions) | **Completed, frozen** | `src/ingestion_bench/graph_retrieval_benchmark/*.py` (new package: config/model/extractor/builder/store/postgres_store/retriever/evaluator/report/benchmark_runner), `scripts/run_stage7b1_graph_benchmark.py` (new) | `tests/test_graph_{build,retrieval}.py` (27, new; includes one skippable real OpenAI+Postgres+sentence-transformers build/retrieval test); `reports/stage7b1_{graph_build_results.json,graph_retrieval_results.json,vector_vs_graph_scorecard.md}` (real run: real gpt-4o-mini extraction, real Postgres, real embeddings; committed reports embed full node/edge/provenance for audit); `artifacts/stage7b1/{graph_manifest,graph_nodes,graph_edge_assertions,comparison}.json + extraction_runs/ + query_results/` (gitignored, regenerable) | Real run: frozen input verified; graph authority correct 12/12 (zero leakage); build recall 0.80 / precision 0.86 / 0 collisions / $0.0011. Graph improved 0 questions, regressed 5 -- with a real LLM extractor a single missed/inconsistently-named edge breaks a multi-hop chain, so traversal retrieves LESS than Vector (extraction is also non-deterministic). The deterministic best-case (perfect extractor) DOES improve the deep endpoint multi-hop Q04/Q06/Q07 but regresses mid-chain-seed Q05/Q10 and the unnamed-entity draft Q12 -- so even ideally the benefit is mixed under the frozen budgets + mandated hop-first ranking. **Recommendation: defer Graph -- its benefit does not justify its cost/fragility for this architecture** (full findings + rationale in `docs/STAGE7B1_GRAPH_VS_VECTOR_DECISION.md`). No answer generation / ADK / wiki / vision / Neo4j / router / query decomposition built. **Stage 7B.1 is now frozen** |
| 7B.2a | Hybrid Vector-Graph Retrieval Value Probe: a bounded decision experiment closing the Graph investigation -- tests whether Vector-assisted graph seeding, semantic edge matching, semantic path ranking, and deterministic RRF Vector/Graph fusion can expose Graph's latent multi-hop value WITHOUT enlarging the final evidence budget, adding authority leakage, or a query-time LLM. NEW isolated `hybrid_retrieval_benchmark` package (model/edge_index/seed_provider/path_retriever/fusion/evaluator/report/benchmark_runner/config): loads the committed Stage 7B.1 real graph snapshot (verified against its graph payload SHA-256; NO OpenAI re-extraction) + builds the deterministic perfect FakeRelationshipExtractor graph (verified recall/precision 1.00, 0 collisions); an isolated edge-semantic index (canonical subject+predicate+object+supporting_text, authority-eligible edges ranked with eligibility applied before similarity+LIMIT; in-memory + isolated Postgres); three deterministic seed providers (explicit-alias, Vector-chunk, semantic-edge -- dedup by node preserving all origins, never infers an entity absent a graph assertion); bounded simple-path enumeration (no repeated node, <=5 hops, all seed-reachable eligible simple paths enumerated and ranked before post-score truncation) with semantic PATH ranking (query<->path-representation cosine, not hop distance); deterministic RRF fusion (one global constant) into exactly the frozen per-question top-K. Five frozen modes V/G/H0/H1/H2 over both graph conditions, all scored by the SAME frozen Stage 7B.0 `_evaluate_question`; fixed pre-declared decision gates evaluated in the order A→B→D→C. No query-time LLM (dependency-isolation test) | **Completed, frozen** | `src/ingestion_bench/hybrid_retrieval_benchmark/*.py` (new package), `contracts/hybrid_retrieval_probe_v1.json` (new; immutable global config + gates, no per-question tuning), `scripts/run_stage7b2_hybrid_probe.py` (new), `docs/STAGE7B2_HYBRID_GRAPH_CLOSURE_DECISION.md` (new) | `tests/test_hybrid_probe.py` (32, new; includes skippable real sentence-transformers + persisted-Postgres measured-run and edge/vector eligibility tests); `reports/stage7b2_hybrid_retrieval_{results.json,scorecard.md}` (real ST run); `artifacts/stage7b2/{input_verification,real_graph_manifest,perfect_graph_manifest,edge_embedding_index_manifest,ablation_comparison,final_decision}.json + query_results/{real_graph,perfect_graph}/` (gitignored, regenerable) | Real run: frozen inputs verified; **zero authority leakage across all modes**; final budget never exceeds top-K; no query-time LLM. On the real graph, RRF fusion (H0/H1/H2) **rescues every Graph regression back to Vector's level but improves 0 of Q04/Q06/Q07**; on the PERFECT graph, G alone solves the targets but fusion with the fixed Vector ranking under the same budget **dilutes those gains back to Vector** (semantic seeding H1 / path ranking H2 add nothing over plain fusion H0, and sometimes hurt). **Decision gate D: Do not retain Graph in the online retrieval path; navigation or offline relationship analysis remains a separate, unevaluated use case** (full rationale in `docs/STAGE7B2_HYBRID_GRAPH_CLOSURE_DECISION.md`). Conclusions are scoped to the tested equal-weight RRF implementation, the declared max-4 supplemental-seed budget, the corrected all-paths-before-ranking generation, this corpus, and this embedding model (not a claim that hybrid can never exceed Vector). No answer generation / ADK / wiki / vision / Neo4j / query-planning LLM / router built. **Stage 7B.2a applied** (global supplemental-seed cap + RRF ranking, all authority-eligible simple paths reachable from selected seeds enumerated & semantically ranked before truncation with a 5000 safety ceiling, fixed A→B→D→C gate precedence, isolated-Postgres measured run, frozen-Stage-7B.1-G exact-equality verification, and split timing/call accounting). **Stage 7B.2a is now frozen** |
| 7B | Graph-enriched RAG projection (production) | **Not retained in the online retrieval path for this architecture (navigation/offline analysis remains a separate, unevaluated use case)** | — | — | Stage 7B.1 recommended defer; Stage 7B.2a's hybrid probe then showed that — for the tested equal-weight RRF, the declared max-4 supplemental-seed budget, all-paths-before-ranking generation, this corpus and this embedding model — RRF fusion converged Graph to Vector (rescued a bad graph up to Vector; on the perfect graph, diluted gains under the shared budget) -- **gate D: do not retain Graph in the online retrieval path**. Not a claim that hybrid can never exceed Vector (the same run's perfect-graph H0 raised Q06 0.80→1.00). Revisit only if a larger/noisier corpus (where Vector degrades) or an enlarged evidence budget changes the trade-off |
| 7C.0 | Deterministic Wiki Projection Qualification (W0): builds the source-backed structural substrate the whole Stage 7C experiment stands on, with ZERO LLM calls — `CanonicalChunk` → deterministic identities → anchors → anchor postings → deterministic facet/page membership → structural + exact-anchor links → authority-scoped views → deterministic rendering. Three anchor lanes over `source_text`/`heading_path` only (Lane 1 identifier regex LIFTED into a neutral module rather than imported from the frozen graph package; Lane 2 conservative repeated-phrase with a closed stop-list and a ≥2-distinct-document rule; Lane 3 heading-title, which deliberately creates NO page identity). Membership is a pure function of page identities + anchor postings, so no future claim/alias/summary/validation outcome/adjudication verdict can alter it. Build time never calls the resolver and stores no authority state | **Completed, frozen** | `src/ingestion_bench/wiki_projection/{__init__,config,identity,model,projection,store,pg_store,rendering,benchmark,report}.py` (new package), `contracts/wiki_projection_v1.json` (new; frozen projection/identity/membership rules, sentence splitter, D0 seed + prioritizer contracts, measured `M_max`), `scripts/run_stage7c0_wiki_projection.py` (new) | `tests/test_wiki_projection.py` + `tests/test_wiki_projection_integration.py` (69, new; membership independence, C-88/C-88a separation, authority-view behaviour, cross-process hash determinism under varying `PYTHONHASHSEED`, zero benchmark-truth and zero Graph runtime dependency); `reports/stage7c0_wiki_projection_{results.json,scorecard.md,sample_pages.md}` (real sentence-transformers run); `artifacts/stage7c0/` (gitignored, regenerable) | 6 documents / 11 revisions / 11 sections; 25 anchors (12 identifier, 1 phrase, 12 heading-title), 47 postings, 13 page identities, **22 facets**, 34 structural + 48 exact-anchor links (6 advisory); **`M_max` = 3 measured and frozen** for the Revision 6 ceiling `C = (P_seed + B) × M_max × F_max` (that ceiling is NOT evaluated in this stage). Only `Payment Settlement` survives Lane 2, matching the plan's own §0.1 corpus property. **W0 semantic control == V on 12/12 questions** with real `all-MiniLM-L6-v2`, reproducing the frozen Stage 7B.0 outcomes exactly (Q04 partial 0.50; Q06/Q07 partial 0.80; nine solved) and scored by the frozen 7B.0 `_evaluate_question` imported by identity; **zero authority leakage**. W0 ≈ V is the contracted SUCCESSFUL control outcome and is explicitly not D0. An authority change alters only the view — no reparse, rechunk, anchor rebuild, embedding rebuild or hash change. D0 seed + prioritizer contracts and the sentence splitter are frozen but deliberately **not executed**. Four interpretation calls on ambiguous frozen plan text (Lane 2 stop-list semantics, Lane 2 identifier collision, `display_title` surface form, rendering module placement) are recorded in the scorecard rather than resolved silently |
| 7C.1 | Bounded, source-grounded W1 facet compiler, run to the **OWNER-ADJUDICATION CHECKPOINT** only: one frozen compiler treatment over the frozen 7C.0 projection, model output limited to exactly three fields (`aliases`, `claims`, `summary_sentences`), every Revision 6 §4.1 deterministic rule applied, and §3.7 claim-derived links derived from accepted claims. Claude is deliberately NOT the adjudicator — citation validity is not claim correctness, summary reference validity is not summary correctness, alias span validity is not alias semantic correctness (§4.3–4.5), so all three judgements are handed over undecided | **Checkpoint reached; awaiting owner adjudication — NOT frozen** | `src/ingestion_bench/wiki_projection/{compiler,validation,assembly,adjudication}.py` (new), `scripts/run_stage7c1_wiki_compiler.py` (new; hard-STOPs before the first call if §3.9's whole-run dollar ceiling is unresolved or model parity fails), `scripts/analyze_stage7c1_repeatability.py` (new; read-only) | `tests/test_wiki_compiler.py` (48, new) + `tests/test_wiki_validation.py` (40, new); `reports/stage7c1_compilation_runs.json` (the frozen Runs 1/2/3 — tracked deliberately, because `artifacts/` is gitignored and these runs are NOT regenerable), `reports/stage7c1_owner_adjudication_packet.{json,md}`, `reports/stage7c1_checkpoint_results.json`, `reports/stage7c1_w1_page_previews.md`, `reports/stage7c1_repeatability_analysis.{md,json}`; `artifacts/stage7c1/` (gitignored) | 66 real `gpt-4o-mini` @ `temperature = 0` calls (22 facets × 3 runs, the frozen Stage 7B.1 extraction model per §3.8 parity), **$0.0188** against the owner's $5.00 cap, 62–70 s per run, zero generation failures, zero ceiling breaches, membership re-verified unchanged. Run 1 designated primary BEFORE execution. Run 1: 26 claims (25 accepted, 0 rejected, 0 uncertain, 1 `out_of_page_scope`), 21 supported aliases, 22 summary sentences, 34 derived links, 1 facet with zero accepted claims (still fully navigable). **68 items await owner verdicts; ZERO facet embeddings created** — §6.2 components 2 and 7 are owner-dependent and §4.6 writes embeddings only after pass 3. **Gate Q = PENDING OWNER ADJUDICATION**, with Q-8 repeatability breaching its proposed threshold. **Repeatability, corrected** (`docs`-linked analysis below): accepted-claim pairwise Jaccard min **0.6207** (< 0.90 — breach stands) but citation exact-agreement on matched claims **1.0000** (≥ 0.95 — the originally reported citation breach was a metric artifact, not a real failure). 15 of 22 facets are byte-identical across all three runs; variance concentrates in `adj_rev1`, the corpus's only multi-sentence chunk, which hosts every entity and predicate change. Two gaps remain **pending a Stage 7C.1B decision and are NOT implemented**: (1) the §8F metric correction, and (2) pass-3 claim withdrawal — §4.6's "nothing that failed adjudication reaches a vector, a summary, or a derived link" is implemented for the alias cascade and summary withdrawal but not for claims, which makes all 25 accepted-claim verdicts load-bearing for the final W1-D/W1-FULL representation and routing |
| 7C.2 | Wiki Hub Retrieval / Navigation Qualification: the three frozen truth-free arms (D0 / W1-D / W1-FULL), the attribution deltas, the counterfactual resilience probe, and the Gate A/B/C decision | **Blocked — not started** | — | — | Preconditions unmet per Revision 6 §0: the owner adjudication verdict set, the final post-adjudication accepted claim set, the final W1 facet embedding manifest/hash, and the final Gate Q result do not exist. D0 consumes no W1-derived model output and is runnable today; **W1-D and W1-FULL are not**, because both seed from the post-adjudication W1 facet embeddings. Not started rather than partially built, because §4's fairness contract requires all three arms to share one configuration |
| 7C | Wiki page/link projection | **In progress — 7C.0 frozen, 7C.1 at the owner-adjudication checkpoint, 7C.2 blocked** | — | — | Experimental contract frozen as `docs/STAGE7C_WIKI_PLAN.md` **Revision 6** (owner-approved). R6's core correction: R5 compared `N_W0` against `N_W1` while holding the W1 enriched facet seed constant in BOTH arms, so it could isolate claim-derived routing but could never show the compiler was unnecessary — the compiler was seeding both sides. R6 adds the **D0** arm (chunk-embedding seed, no W1-derived model output) and names three arms **D0 / W1-D / W1-FULL**; Gate A-7 now requires W1-FULL to beat **D0**, and Gate B may never be selected from `W1-FULL ≈ W1-D` alone. Terminology: never write "zero-model" for D0 — it uses the existing embedding model; use **zero-W1-LLM**, **no W1-derived model output**, or **deterministic D0** |
...
- Wiki page/link retrieval (Stage 7C — **in progress**, consuming Stage 7R's resolver at query time: 7C.0 deterministic projection **frozen**; 7C.1 bounded W1 compiler **at the owner-adjudication checkpoint**, 68 verdicts outstanding and no facet embedding built; 7C.2 retrieval/navigation **blocked** on those verdicts)
...
Stage 7C       Wiki page/link projection                           <- IN PROGRESS
               7C.0 deterministic projection (W0)                  <- DONE, FROZEN
               7C.1 bounded W1 compiler                            <- AWAITING OWNER ADJUDICATION
               7C.2 retrieval/navigation qualification             <- BLOCKED on 7C.1 verdicts
