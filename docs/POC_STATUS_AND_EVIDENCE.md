...
| 7R.2 | Authority-Aware Vector Retrieval: proves Stage 7R.1's registry/resolver actually changes semantic-search results. Isolated benchmark over ONE logical document (POLICY-RETENTION-001, 5 near-identical real DOCX revisions through the frozen Stage 5A adapter + Stage 4/4.1 chunker), an isolated Postgres/pgvector table (never Stage 7A.1's own), a narrow authority-aware retriever (resolve_query_scope() -> fail closed on integrity_error -> SQL-level `WHERE document_revision_id IN (...)` eligibility filter BEFORE `ORDER BY`/`LIMIT` -- never post-filtered), current/historical/draft/comparison scenarios plus a live authority-switch scenario proving zero re-ingestion/re-chunking/re-embedding across an activation, and an honest unfiltered-vs-authority-aware comparison (leakage@K/precision@K/hit@K) | **Completed** | `src/ingestion_bench/revision_search_benchmark/*.py` (new package), `fixtures/revision_search/*` (new), `contracts/revision_search_benchmark_v1.json` (new), `scripts/run_stage7r2_authority_aware_benchmark.py` (new) | `tests/test_revision_search_*.py` (21, new; includes one skippable real sentence-transformers+pgvector integration test); `reports/stage7r2_authority_aware_vector_{results.json,scorecard.md}`, `artifacts/stage7r2/{index_manifest,registry_before,registry_after}.json` + `query_results/*.json` (real run: real Docling conversion, real sentence-transformers, real Postgres) | None known -- unfiltered retrieval genuinely leaks ineligible revisions (leakage@K 6-8 of 10) even with the real embedding model, since the five fixtures are deliberately near-identical; authority-aware precision@K is 1.00 in every scenario |
