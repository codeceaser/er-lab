...
| 7B | Graph-enriched RAG projection (production) | **Deferred** | — | — | Stage 7B.1 measured Graph vs Vector and recommends **defer**: benefit is latent (perfect extraction improves deep multi-hop) but not reliably achievable with a real, non-deterministic LLM extractor, and it adds cost/maintenance. Revisit only if a higher-recall/deterministic extractor or a larger corpus changes the economics |
